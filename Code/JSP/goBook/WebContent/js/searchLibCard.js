function detailsForm(realname, ugender, ubirth, uphone, wechat, qq, uaddress, uidcard, applydate, processtime, process, inuse) {
	
	$("#realname").val(realname);
	$("#ugender").val(ugender);
	$("#ubirth").val(ubirth);
	$("#updateUphone").val(uphone);
	$("#updateWechat").val(wechat);
	$("#updateQq").val(qq);
	$("#updateUaddress").val(uaddress);
	$("#idcard").val(uidcard);
	$("#uidcard").val(uidcard);
...
window.alertCancel = function(str)
{
    ...
    strHtml += " <li style=\"background:RGBA(85,85,85,0.9);text-align:center;height:30px;line-height:30px;border-bottom-left-radius: 4px;border-bottom-right-radius: 4px;border-top:1px solid #CCCCCC;\">" +
    		"<input type=\"button\" value=\"确 定\" onclick=\"cancelLibCard()\" style=\"position:absolute;top:126px;left:45px;width:80px;height:20px;background:#626262;color:white;border-radius:3px;border:1px solid #CCCCCC;font-size:14px;\" onmouseover=\"this.style.background='#343434'\" onmouseout=\"this.style.background=0\" />" +
    ...
    this.cancelLibCard = function(){
    	window.location.href = "../root/cancelLibCard?uidcard="+uidcard;
    }
    alertFram.focus();
