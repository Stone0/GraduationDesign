//确认完成预约消息框
window.alertSubComplete = function(str,sid,isbn,uidcard)
{
    ...
    strHtml += " <li style=\"background:RGBA(85,85,85,0.9);text-align:center;height:30px;line-height:30px;border-bottom-left-radius: 4px;border-bottom-right-radius: 4px;border-top:1px solid #CCCCCC;\">" +
    		"<input type=\"button\" value=\"确 定\" onclick=\"subComplete()\" style=\"position:absolute;top:126px;left:45px;width:80px;height:20px;background:#626262;color:white;border-radius:3px;border:1px solid #CCCCCC;font-size:14px;\" onmouseover=\"this.style.background='#343434'\" onmouseout=\"this.style.background=0\" />" +
    ...
    this.subComplete = function(){
    	window.location.href = "../manager/subComplete?sid="+sid+"&isbn="+isbn+"&uidcard="+uidcard;
    }
    alertFram.focus();
